...
async function generateReport() {
  ...
  if (fs.existsSync(path.join(tmpDir, 'qa-apply-fixes-log.json'))) {
    fixesLog = JSON.parse(
      fs.readFileSync(path.join(tmpDir, 'qa-apply-fixes-log.json'), 'utf-8')
    );
  }
  ...
  const fullReport: FullReport = {
    ...
    statistics: {
      ...
      needs_manual_review: structureReport.summary.recipes_flagged - (fixesLog ? fixesLog.applied_fixes : 0),
    ...
    by_issue_type: {
      ...
      empty_strings: structureReport.warnings.empty_strings_in_ingredients.length +
  ...
  summaryLines.push('─'.repeat(70));
  summaryLines.push(`✅ Validated (No Issues):        ${formatNumber(fullReport.statistics.validated)} (${formatPercentage(fullReport.statistics.validated, fullReport.total_recipes)})`);
  summaryLines.push(`🔧 Fixed Automatically:          ${formatNumber(fullReport.statistics.fixed_automatically)} (${formatPercentage(fullReport.statistics.fixed_automatically, fullReport.total_recipes)})`);
  summaryLines.push(`⚠️  Needs Manual Review:          ${formatNumber(fullReport.statistics.needs_manual_review)} (${formatPercentage(fullReport.statistics.needs_manual_review, fullReport.total_recipes)})`);
  summaryLines.push(`🔴 Critical Issues:              ${formatNumber(fullReport.statistics.critical_issues)}`);
  summaryLines.push('');
  summaryLines.push('─'.repeat(70));
  summaryLines.push('ISSUES BY TYPE');
  summaryLines.push('─'.repeat(70));
  summaryLines.push(`Missing Ingredients:             ${formatNumber(fullReport.by_issue_type.missing_ingredients)}`);
  summaryLines.push(`Missing Instructions:            ${formatNumber(fullReport.by_issue_type.missing_instructions)}`);
  summaryLines.push(`Ingredient Mismatch:             ${formatNumber(fullReport.by_issue_type.ingredient_mismatch)}`);
  summaryLines.push(`Malformed JSON:                  ${formatNumber(fullReport.by_issue_type.malformed_json)}`);
  summaryLines.push(`Empty Strings in Arrays:         ${formatNumber(fullReport.by_issue_type.empty_strings)}`);
  summaryLines.push('');
  ...
  if (derivationReport) {
    ...
    summaryLines.push('─'.repeat(70));
    summaryLines.push(`🟢 High Confidence (≥0.90):     ${formatNumber(fullReport.by_confidence.high_0_90_plus)}`);
    summaryLines.push(`🟡 Medium Confidence (0.70-0.89): ${formatNumber(fullReport.by_confidence.medium_0_70_to_0_90)}`);
    summaryLines.push(`🔴 Low Confidence (<0.70):       ${formatNumber(fullReport.by_confidence.low_below_0_70)}`);
    summaryLines.push('');
  }
  ...
  if (extractionReport) {
    ...
    summaryLines.push('─'.repeat(70));
    summaryLines.push(`Perfect Matches (100%):          ${formatNumber(extractionReport.statistics.perfect_matches)}`);
    summaryLines.push(`High Matches (≥80%):             ${formatNumber(extractionReport.statistics.high_matches)}`);
    summaryLines.push(`Medium Matches (≥60%):           ${formatNumber(extractionReport.statistics.medium_matches)}`);
    summaryLines.push(`Low Matches (<60%):              ${formatNumber(extractionReport.statistics.low_matches)}`);
    summaryLines.push(`Extraction Errors:               ${formatNumber(extractionReport.statistics.extraction_errors)}`);
    summaryLines.push('');
  }
  ...
  if (derivationReport) {
    const lowConfResults = derivationReport.all_results.filter(
      (r: any) => r.confidence < 0.90 && r.confidence > 0
    );
