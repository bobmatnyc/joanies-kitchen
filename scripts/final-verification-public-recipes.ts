...
async function finalVerification() {
  ...
  try {
    ...
    console.log(
      `   Public recipes: ${overallStats[0].publicRecipes} (${((overallStats[0].publicRecipes / overallStats[0].total) * 100).toFixed(1)}%)`
    );
    console.log(
      `   Private recipes: ${overallStats[0].privateRecipes} (${((overallStats[0].privateRecipes / overallStats[0].total) * 100).toFixed(1)}%)`
    );
    ...
    console.log(
      `   ${chefStatus} Chef recipes: ${chefStats[0].total} total, ${chefStats[0].public} public, ${chefStats[0].private} private`
    );
    ...
    console.log(
      `   ${systemStatus} System recipes: ${systemStats[0].total} total, ${systemStats[0].public} public, ${systemStats[0].private} private`
    );
    ...
    chefBreakdown.forEach((chef) => {
      ...
      console.log(
        `   ${status} ${chef.chefName}: ${chef.recipeCount} recipes (${chef.publicCount} public, ${privateCount} private)`
      );
    });
