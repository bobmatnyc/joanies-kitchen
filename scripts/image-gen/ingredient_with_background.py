...
def get_random_background() -> Optional[Path]:
    ...
    backgrounds = list(BACKGROUNDS_DIR.glob("*.JPG")) + list(BACKGROUNDS_DIR.glob("*.jpg"))
    if not backgrounds:
...
def random_crop_and_zoom(image: Image.Image, zoom: float, target_size: Tuple[int, int]) -> Image.Image:
    """
    ...
    # Crop and resize
    cropped = image.crop((x_offset, y_offset, x_offset + crop_width, y_offset + crop_height))
...
def create_product_shot_with_background(
    ...
    print(f"   🎨 Effects: zoom={zoom:.2f}x, blur={blur_radius:.1f}px, rotation={rotation:.1f}°, brightness={brightness:.2f}")
...
def composite_item_on_background(item: Image.Image, background: Image.Image) -> Image.Image:
...
def main():
    parser = argparse.ArgumentParser(description="Generate ingredient/tool images with kitchen backgrounds")
    parser.add_argument("--item", help="Item name (e.g., 'olive oil')")
    parser.add_argument("--category", help="Category (e.g., 'oils', 'tools')")
    parser.add_argument("--output", help="Output file path")
    parser.add_argument("--batch", help="Batch file with items (one per line: item|category)")
    parser.add_argument("--no-sd", action="store_true", help="Don't use Stable Diffusion (backgrounds only)")
    ...
    if args.batch:
        ...
        with open(args.batch) as f:
            for line in f:
                ...
                item, category = parts
                output_path = OUTPUT_DIR / category / f"{item.lower().replace(' ', '-')}.png"
        ...
        if args.output:
            ...
            output_path = OUTPUT_DIR / args.category / f"{args.item.lower().replace(' ', '-')}.png"
