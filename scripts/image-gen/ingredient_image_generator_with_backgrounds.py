...
class EnhancedIngredientImageGenerator:
    ...
    def _build_ingredient_prompt(self, ingredient_name: str) -> tuple[str, str]:
        ...
        fresh_produce = [
            'lettuce', 'spinach', 'kale', 'arugula', 'tomato', 'cucumber',
            'bell pepper', 'carrot', 'celery', 'apple', 'grape', 'berry',
            'strawberry', 'blueberry', 'raspberry', 'blackberry', 'cherry',
            'orange', 'lemon', 'lime', 'peach', 'plum', 'nectarine', 'watermelon'
        ]
        has_droplets = any(produce in ingredient_name.lower() for produce in fresh_produce)

        # Determine portion size and presentation
        is_herb = any(h in ingredient_name.lower() for h in ["herb", "basil", "parsley", "cilantro", "thyme", "rosemary", "mint", "oregano", "sage"])
        is_packaged = any(p in ingredient_name.lower() for p in ["milk", "cream", "bottle", "can", "package", "box", "jar"])
        ...
        if is_herb:
            ...
            surface = random.choice(["kitchen counter", "marble countertop", "induction cooktop"])
        else:
    ...
    def generate(
        ...
        for i in range(num_images):
            ...
            if init_image is None:
                print(f"  ⚠️  No backgrounds available, skipping...")
                continue
...
def main():
    ...
    parser.add_argument(
        '--ingredient',
        type=str,
        help='Single ingredient name to generate'
    )
    parser.add_argument(
        '--batch',
        type=Path,
        help='File with ingredient names (one per line)'
    )
    parser.add_argument(
        '--output-dir',
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help='Number of images per ingredient'
    )
    ...
    generator = EnhancedIngredientImageGenerator(
