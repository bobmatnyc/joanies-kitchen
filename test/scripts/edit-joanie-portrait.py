...
def change_eye_color_to_ocean_blue(image: np.ndarray, eye_regions: list) -> np.ndarray:
    ...
    for x, y, w, h in eye_regions:
        ...
        eye_region[:, :, 1] = np.where(
            final_mask,
            np.clip(eye_region[:, :, 1] + target_saturation - 100, 0, 255).astype(
                np.uint8
            ),
            eye_region[:, :, 1],
        )
        ...
        for i in range(3):  # For each HSV channel
            eye_region[:, :, i] = np.where(
                final_mask, eye_region_blurred[:, :, i], hsv[y : y + h, x : x + w, i]
            )
...
def enhance_lips(image: np.ndarray, lip_region: tuple) -> np.ndarray:
    ...
    hsv_enhanced[:, :, 1] = np.clip(hsv_enhanced[:, :, 1] * 1.05, 0, 255).astype(
        np.uint8
    )
    enhanced_lip = cv2.cvtColor(hsv_enhanced, cv2.COLOR_HSV2BGR)

    # Blend with original using alpha
    lip_area = (lip_area * (1 - alpha * 0.3) + enhanced_lip * (alpha * 0.3)).astype(
        np.uint8
    )
...
def main():
    ...
    lip_region = (face_center_x - lip_width // 2, lip_y, lip_width, lip_height)
