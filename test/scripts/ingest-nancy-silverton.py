...
OUTPUT_DIR = (
    Path(__file__).parent.parent / "data" / "recipes" / "incoming" / "nancy-silverton"
)
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("NANCY SILVERTON RECIPE SCRAPER (FOOD & WINE)")
    logger.info("=" * 70)
    ...
    if success_rate >= 80:  # Lower threshold for Food & Wine (may have more issues)
        logger.success(
            f"🎉 SUCCESS! Scraping complete with {success_rate:.1f}% success rate"
        )
        logger.info("\n📁 Output files:")
        ...
        logger.warning(
            f"⚠️  Completed with {success_rate:.1f}% success rate (target: 80%)"
        )
