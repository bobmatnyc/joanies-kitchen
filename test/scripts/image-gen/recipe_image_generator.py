...
class RecipeImageGenerator:
    ...
    def _load_model(self):
        ...
        self.pipe = StableDiffusionXLPipeline.from_pretrained(
            self.config.model_id, torch_dtype=self.config.dtype, use_safetensors=True
        )
    ...
    def save_images(
        self, images: List[Image.Image], recipe_name: str, output_dir: Path
    ) -> List[Path]:
...
def main():
    ...
    parser.add_argument("--recipe", type=str, help="Recipe name to generate image for")
    parser.add_argument("--style", type=str, help="Custom style description (optional)")
    parser.add_argument(
        "--negative", type=str, help="Negative prompt (things to avoid)"
    )
    parser.add_argument(
        "--num-images",
        type=int,
    ...
    parser.add_argument(
        "--batch", type=Path, help="Batch file with recipe names (one per line)"
    )
    parser.add_argument(
        "--steps", type=int, default=30, help="Number of inference steps (default: 30)"
    )
    ...
    # Create config
    config = ImageGenerationConfig(num_inference_steps=args.steps)
    ...
    if args.recipe:
        try:
            ...
            saved_paths = generator.save_images(images, args.recipe, args.output_dir)
    ...
    # Batch generation
        ...
        for i, recipe in enumerate(recipes, 1):
            ...
            try:
                images = generator.generate(
                    recipe_name=recipe, style=args.style, num_images=args.num_images
                )
