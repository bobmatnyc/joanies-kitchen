...
def transform_eyes_to_blue(image_path, output_path):
    ...
    left_eye_mask = (
        ((x_coords - left_eye_center[0]) / left_eye_radius[0]) ** 2
        + ((y_coords - left_eye_center[1]) / left_eye_radius[1]) ** 2
    ) <= 1

    right_eye_mask = (
        ((x_coords - right_eye_center[0]) / right_eye_radius[0]) ** 2
        + ((y_coords - right_eye_center[1]) / right_eye_radius[1]) ** 2
    ) <= 1
    ...
    lip_mask = (
        ((x_coords - lip_center[0]) / lip_radius[0]) ** 2
        + ((y_coords - lip_center[1]) / lip_radius[1]) ** 2
    ) <= 1
    ...
    print("   - Eyes: Aggressive ocean blue replacement (hue=210°, high saturation)")
    print("   - Lips: Enhanced color and depth")
