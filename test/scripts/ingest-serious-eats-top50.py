...
OUTPUT_DIR = (
    Path(__file__).parent.parent / "data" / "recipes" / "incoming" / "serious-eats"
)
...
def scrape_recipe(
    url: str, metadata: Dict[str, str], attempt: int = 1
...
def main():
    ...
    for raw_data in raw_recipes:
        ...
        if is_valid:
            ...
            logger.warning(
                f"Validation issues for {transformed['name']}: {', '.join(issues)}"
            )
    ...
    if validation_failures:
        logger.warning(
            f"  Issues found: {len(validation_failures)}/{len(transformed_recipes)}"
        )

    # Quality checks
    logger.info("\n📊 Quality Checks:")
    recipes_with_images = sum(
        1 for r in transformed_recipes if json.loads(r.get("images", "[]"))
    )
    ...
    logger.info(
        f"  With prep time: {recipes_with_prep_time}/{len(transformed_recipes)}"
    )
    logger.info(
        f"  With cook time: {recipes_with_cook_time}/{len(transformed_recipes)}"
    )
    ...
    if success_rate >= 90:
        ...
        logger.warning(
            f"⚠️  Completed with {success_rate:.1f}% success rate (target: 90%)"
        )
