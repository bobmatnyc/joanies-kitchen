...
async function countImageTypes() {
  ...
  console.log(
    `Unsplash (external): ${stats[0].unsplash} (${((Number(stats[0].unsplash) / Number(stats[0].total)) * 100).toFixed(1)}%)`
  );
  console.log(
    `Local files: ${stats[0].local} (${((Number(stats[0].local) / Number(stats[0].total)) * 100).toFixed(1)}%)`
  );
