...
def test_scraper(recipe_info):
    ...
    try:
        ...
        print(
            f"Image: {data['image'][:80] + '...' if data['image'] and len(data['image']) > 80 else data['image']}"
        )
        ...
        instructions_preview = (
            data["instructions"][:150] + "..."
            if len(data["instructions"]) > 150
...
def main():
    ...
    for recipe_info in TEST_RECIPES:
        ...
        results.append(
            {"recipe": recipe_info["name"], "url": recipe_info["url"], **result}
        )
