...
import { EmbeddingError, generateChefEmbedding } from '@/lib/ai/embeddings';
import { db } from '@/lib/db';
import { countChefEmbeddings, saveChefEmbedding } from '@/lib/db/meal-chef-embeddings';
...
async function main() {
  ...
  if (stats.failed > 0) {
    ...
    log(
      `\n   Error log saved to: ${path.join(tmpDir, 'chef-embedding-errors.log')}`,
      colors.yellow
    );
